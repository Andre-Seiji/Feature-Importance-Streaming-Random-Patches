/*
 *    DSFeatureImportanceStreamingRandomPatches.java
 *
 *
...
public class DSFeatureImportanceStreamingRandomPatches extends AbstractClassifier implements MultiClassClassifier,
        CapabilitiesHandler, FeatureImportanceClassifier {

    private static final long serialVersionUID = 1L;

    public ClassOption baseLearnerOption = new ClassOption("baseLearner", 'l',
            "Classifier to train on instances.", Classifier.class, "trees.DSFISRPHoeffdingTree -g 50 -c 0.01");
    ...
    protected void initEnsemble(Instance instance) {
        ...
        if(this.trainingMethodOption.getChosenIndex() != DSFeatureImportanceStreamingRandomPatches.TRAIN_RESAMPLING) {
            ...
            switch(this.subspaceModeOption.getChosenIndex()) {
                case DSFeatureImportanceStreamingRandomPatches.FEATURES_SQRT:
                    k = (int) Math.round(Math.sqrt(n)) + 1;
                    subspaceModeSelected = 1;
                    break;
                case DSFeatureImportanceStreamingRandomPatches.FEATURES_SQRT_INV:
                    k = n - (int) Math.round(Math.sqrt(n) + 1);
                    subspaceModeSelected = 2;
                    break;
                case DSFeatureImportanceStreamingRandomPatches.FEATURES_PERCENT:
            ...
            if(this.trainingMethodOption.getChosenIndex() == DSFeatureImportanceStreamingRandomPatches.TRAIN_RANDOM_SUBSPACES ||
                    this.trainingMethodOption.getChosenIndex() == DSFeatureImportanceStreamingRandomPatches.TRAIN_RANDOM_PATCHES) {
                if(k != 0 && k < n) {
                    ...
                    if(n <= 20 || k < 2) {
                        ...
                        this.subspaces = DSFeatureImportanceStreamingRandomPatches.allKCombinations(k, n);
                    ...
                    // subspaces without worrying about repetitions.
                        ...
                        this.subspaces = DSFeatureImportanceStreamingRandomPatches.localRandomKCombinations(k, n,
                ...
                // k == 0 or k > n (subspace size greater than the total number of features), then default to resampling
                    ...
                    this.trainingMethodOption.setChosenIndex(DSFeatureImportanceStreamingRandomPatches.TRAIN_RESAMPLING);
                }
            }
        }
        ...
        for(int i = 0 ; i < ensembleSize ; ++i) {
            switch(this.trainingMethodOption.getChosenIndex()) {
                case DSFeatureImportanceStreamingRandomPatches.TRAIN_RESAMPLING:
                    this.ensemble[i] = new StreamingRandomPatchesClassifier(
                            ...
                            false);
                    break;
                case DSFeatureImportanceStreamingRandomPatches.TRAIN_RANDOM_SUBSPACES:
                case DSFeatureImportanceStreamingRandomPatches.TRAIN_RANDOM_PATCHES:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == DSFeatureImportanceStreamingRandomPatches.class)
