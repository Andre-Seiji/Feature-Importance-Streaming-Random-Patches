 ...
 * @version $Revision: 1 $
 */
public class DSStreamingRandomPatches extends AbstractClassifier implements MultiClassClassifier,
        CapabilitiesHandler {
    ...
    protected void initEnsemble(Instance instance) {
        ...
        if(this.trainingMethodOption.getChosenIndex() != DSStreamingRandomPatches.TRAIN_RESAMPLING) {
            ...
            switch(this.subspaceModeOption.getChosenIndex()) {
                case DSStreamingRandomPatches.FEATURES_SQRT:
                    k = (int) Math.round(Math.sqrt(n)) + 1;
                    subspaceModeSelected = 1;
                    break;
                case DSStreamingRandomPatches.FEATURES_SQRT_INV:
                    k = n - (int) Math.round(Math.sqrt(n) + 1);
                    subspaceModeSelected = 2;
                    break;
                case DSStreamingRandomPatches.FEATURES_PERCENT:
            ...
            if(this.trainingMethodOption.getChosenIndex() == DSStreamingRandomPatches.TRAIN_RANDOM_SUBSPACES ||
                    this.trainingMethodOption.getChosenIndex() == DSStreamingRandomPatches.TRAIN_RANDOM_PATCHES) {
                if(k != 0 && k < n) {
                    ...
                    if(n <= 20 || k < 2) {
                        ...
                        this.subspaces = DSStreamingRandomPatches.allKCombinations(k, n);
                    ...
                    // subspaces without worrying about repetitions.
                        ...
                        this.subspaces = DSStreamingRandomPatches.localRandomKCombinations(k, n,
                ...
                // k == 0 or k > n (subspace size greater than the total number of features), then default to resampling
                    ...
                    this.trainingMethodOption.setChosenIndex(DSStreamingRandomPatches.TRAIN_RESAMPLING);
                }
            }
        }
        ...
        for(int i = 0 ; i < ensembleSize ; ++i) {
            switch(this.trainingMethodOption.getChosenIndex()) {
                case DSStreamingRandomPatches.TRAIN_RESAMPLING:
                    this.ensemble[i] = new StreamingRandomPatchesClassifier(
                            ...
                            false);
                    break;
                case DSStreamingRandomPatches.TRAIN_RANDOM_SUBSPACES:
                case DSStreamingRandomPatches.TRAIN_RANDOM_PATCHES:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == DSStreamingRandomPatches.class)
