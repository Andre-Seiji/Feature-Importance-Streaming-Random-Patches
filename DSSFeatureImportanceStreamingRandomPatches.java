 ...
 * @version $Revision: 1 $
 */
public class DSSFeatureImportanceStreamingRandomPatches extends AbstractClassifier implements MultiClassClassifier,
        CapabilitiesHandler, FeatureImportanceClassifier {

    private static final long serialVersionUID = 1L;

    public ClassOption baseLearnerOption = new ClassOption("baseLearner", 'l',
            "Classifier to train on instances.", Classifier.class, "trees.DSSFISRPHoeffdingTree -g 50 -c 0.01");
    ...
    public int subspaceModeSelected = 0;
    ...
    public static ArrayList<Integer> initialAttributes = new ArrayList<Integer>();
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        if (previousFeaturesArray != null){
            ...
            if (!previousFeaturesArray.equals(preserveFeaturesArray)) {
                ...
                // Limits changedFeaturesArray size
                int count=0;
                ...
                int featurestopreserve = (int) Math.ceil(count * ((double) maxRelevantFeaturesPercentageOption.getValue()/100));
        ...
        ++this.instancesSeen;
        //System.out.println("instancesSeen: " + this.instancesSeen);
    ...
    protected void initEnsemble(Instance instance) {
        //System.out.println("initEnsemble");
        // Init the ensemble.
        ...
        if(this.trainingMethodOption.getChosenIndex() != DSSFeatureImportanceStreamingRandomPatches.TRAIN_RESAMPLING) {
            ...
            ArrayList<Integer> n_Attributes = new ArrayList<Integer>();
            ...
            // Remove Class
            n_Attributes.remove(n_Attributes.size() -1);
            initialAttributes = n_Attributes;
            int n = n_Attributes.size();

            switch(this.subspaceModeOption.getChosenIndex()) {
                case DSSFeatureImportanceStreamingRandomPatches.FEATURES_SQRT:
                    k = (int) Math.round(Math.sqrt(n)) + 1;
                    subspaceModeSelected = 1;
                    break;
                case DSSFeatureImportanceStreamingRandomPatches.FEATURES_SQRT_INV:
                    k = n - (int) Math.round(Math.sqrt(n) + 1);
                    subspaceModeSelected = 2;
                    break;
                case DSSFeatureImportanceStreamingRandomPatches.FEATURES_PERCENT:
                    double percent = k < 0 ? (100 + k)/100.0 : k / 100.0;
                    k = (int) Math.round(n * percent);
                    subspaceModeSelected = 3;
            ...
            if(this.trainingMethodOption.getChosenIndex() == DSSFeatureImportanceStreamingRandomPatches.TRAIN_RANDOM_SUBSPACES ||
                    this.trainingMethodOption.getChosenIndex() == DSSFeatureImportanceStreamingRandomPatches.TRAIN_RANDOM_PATCHES) {
                if(k != 0 && k < n) {
                    ...
                    if(n <= 20 || k < 2) {
                        ...
                        this.subspaces = DSSFeatureImportanceStreamingRandomPatches.allKCombinations(k, n);
                    ...
                    // subspaces without worrying about repetitions.
                        ...
                        this.subspaces = DSSFeatureImportanceStreamingRandomPatches.localRandomKCombinations(k, n,
                ...
                // k == 0 or k > n (subspace size greater than the total number of features), then default to resampling
                    ...
                    this.trainingMethodOption.setChosenIndex(DSSFeatureImportanceStreamingRandomPatches.TRAIN_RESAMPLING);
                }
            }
        }
        ...
        for(int i = 0 ; i < ensembleSize ; ++i) {
            switch(this.trainingMethodOption.getChosenIndex()) {
                case DSSFeatureImportanceStreamingRandomPatches.TRAIN_RESAMPLING:
                    this.ensemble[i] = new StreamingRandomPatchesClassifier(
                            ...
                            false);
                    break;
                case DSSFeatureImportanceStreamingRandomPatches.TRAIN_RANDOM_SUBSPACES:
                case DSSFeatureImportanceStreamingRandomPatches.TRAIN_RANDOM_PATCHES:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == DSSFeatureImportanceStreamingRandomPatches.class)
    ...
    public static ArrayList<ArrayList<Integer>> localRandomKCombinations(int k, int length,
        ...
        for(int i = 0 ; i < nCombinations ; ++i) {
            ...
            for(int j = 0 ; j < length ; ++j)
                //combination.add(j);
                combination.add(initialAttributes.get(j));
    ...
    private static void allKCombinationsInner(int offset, int k, ArrayList<Integer> combination, long originalSize,
        ...
        for (int i = offset; i <= originalSize - k; ++i) {
            //combination.add(i);
            combination.add(initialAttributes.get(i));
    ...
    protected class StreamingRandomPatchesClassifier {
        ...
        public StreamingRandomPatchesClassifier(int indexOriginal, Classifier instantiatedClassifier,
            ...
            //System.out.println("StreamingRandomPatchesClassifier");
        }
        ...
        public void prepareRandomSubspaceInstance(Instance instance, double weight) {
            ...
            //System.out.println("prepareRandomSubspaceInstance");
        }
        ...
        private ArrayList<Integer> applySubsetResetStrategy(Instance instance, Random random) {
            //System.out.println("applySubsetResetStrategy");
            if(this.subset != null) {
                ...
                fIndexes.remove(fIndexes.size() -1);

                int k = subspaceSizeOption.getValue();
                int n = fIndexes.size();

                switch(subspaceModeSelected) {
                    case 1:
                        k = (int) Math.round(Math.sqrt(n)) + 1;
                        break;
                    case 2:
                ...
                int h = fIndexes.size() - (n - k);

                // Prevent changedFeaturesArray being bigger than fIndexes
                if (changedFeaturesArray != null && changedFeaturesArray.size() >= h){
                        changedFeaturesArray.subList(h, changedFeaturesArray.size()).clear();
                }
        ...
        public void reset(Instance instance, long instancesSeen, Random random) {
            ...
            if(!this.disableBkgLearner && this.bkgLearner != null) {
                ...
                if(this.subset != null) {
                    ...
                    this.featureIndexes = new int[fIndexes.size()];
        ...
        public void triggerWarning(Instance instance, long instancesSeen, Random random) {
            //System.out.println("triggerWarning");
         ...
         */
        public double[] getVotesForInstance(Instance instance) {
            //System.out.println("getVotesForInstance2");
            if(this.subset != null) {
                prepareRandomSubspaceInstance(instance, 1);
