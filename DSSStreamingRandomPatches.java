 ...
 * @version $Revision: 1 $
 */
public class DSSStreamingRandomPatches extends AbstractClassifier implements MultiClassClassifier,
        CapabilitiesHandler {
    ...
    protected void initEnsemble(Instance instance) {
        ...
        if(this.trainingMethodOption.getChosenIndex() != DSSStreamingRandomPatches.TRAIN_RESAMPLING) {
            ...
            switch(this.subspaceModeOption.getChosenIndex()) {
                case DSSStreamingRandomPatches.FEATURES_SQRT:
                    k = (int) Math.round(Math.sqrt(n)) + 1;
                    subspaceModeSelected = 1;
                    break;
                case DSSStreamingRandomPatches.FEATURES_SQRT_INV:
                    k = n - (int) Math.round(Math.sqrt(n) + 1);
                    subspaceModeSelected = 2;
                    break;
                case DSSStreamingRandomPatches.FEATURES_PERCENT:
            ...
            if(this.trainingMethodOption.getChosenIndex() == DSSStreamingRandomPatches.TRAIN_RANDOM_SUBSPACES ||
                    this.trainingMethodOption.getChosenIndex() == DSSStreamingRandomPatches.TRAIN_RANDOM_PATCHES) {
                if(k != 0 && k < n) {
                    ...
                    if(n <= 20 || k < 2) {
                        ...
                        this.subspaces = DSSStreamingRandomPatches.allKCombinations(k, n);
                    ...
                    // subspaces without worrying about repetitions.
                        ...
                        this.subspaces = DSSStreamingRandomPatches.localRandomKCombinations(k, n,
                ...
                // k == 0 or k > n (subspace size greater than the total number of features), then default to resampling
                    ...
                    this.trainingMethodOption.setChosenIndex(DSSStreamingRandomPatches.TRAIN_RESAMPLING);
                }
            }
        }
        ...
        for(int i = 0 ; i < ensembleSize ; ++i) {
            switch(this.trainingMethodOption.getChosenIndex()) {
                case DSSStreamingRandomPatches.TRAIN_RESAMPLING:
                    this.ensemble[i] = new StreamingRandomPatchesClassifier(
                            ...
                            false);
                    break;
                case DSSStreamingRandomPatches.TRAIN_RANDOM_SUBSPACES:
                case DSSStreamingRandomPatches.TRAIN_RANDOM_PATCHES:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == DSSStreamingRandomPatches.class)
